...
def print_matrix(destinations, distance_matrix_km):
    ...
    header = f"{'':<{col_width}}" + "".join(f"{d:<{col_width}}" for d in destinations)
    print(header)
    print("-" * len(header))

    for i, row in enumerate(distance_matrix_km):
        row_str = f"{destinations[i]:<{col_width}}" + "".join(f"{dist:<{col_width}.2f}" for dist in row)
        print(row_str)
...
def interactive_nearest_neighbour_tsp(destinations, distance_matrix_km, neighbours, start_index):
    ...
    print(f"\n--- Final Step: Return to start '{destinations[start_index]}' ({return_distance:.2f} km) ---")
...
def print_step_details(destinations, step_details):
    ...
    for from_node, to_node, dist, was_nearest in step_details:
        note = "nearest option" if was_nearest else "user override"
...
def main():
    ...
    print("(distances verified via Google Maps)")
    ...
    print_header(f"INTERACTIVE WALKTHROUGH FROM '{destinations[start_index].upper()}'")
